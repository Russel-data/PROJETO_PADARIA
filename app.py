...
app.config["SECRET_KEY"] = "MinhaChave1212#"
app.config["SIMPLELOGIN_USERNAME"] = "WALLISSON"
app.config["SIMPLELOGIN_PASSWORD"] = "Pipoca123"
db = SQLAlchemy()
db.init_app(app)
SimpleLogin(app)
...
class Product(db.Model):
    ...
    origem = db.Column(db.String(500))
    ...
    if request.method == "POST":
        ...
        try:
            produto = Product(dados['nome'],
                          ...
                          dados['origem'],
                         imagem.filename)
            imagem.save(os.path.join('static/imagem', imagem.filename))
            db.session.add(produto)
            db.session.commit()
        except:
            status = {"type": "erro", "message": f"Houve algum problema ao cadastrar o {dados['nome']}!"}
...
def editar_produto(id):
    if request.method == 'POST':
        ...
        imagem = request.files['imagem']
        produto = produto_editado = db.session.execute(db.select(Product).filter(Product.id == id)).scalar()
